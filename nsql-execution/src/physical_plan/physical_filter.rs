...
impl<'env, 'txn, S: StorageEngine, M: ExecutionMode<'env, S>> PhysicalFilter<'env, 'txn, S, M> {
    pub(crate) fn plan(
...
impl<'env, 'txn, S: StorageEngine, M: ExecutionMode<'env, S>> Explain<S> for PhysicalFilter<'env, 'txn, S, M> {
    fn explain(
        &self,
        _catalog: &Catalog<S>,
        _tx: &S::Transaction<'_>,
