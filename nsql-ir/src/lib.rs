...
impl fmt::Display for JoinOperator {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            JoinOperator::IsNotDistinctFrom => write!(f, "IS NOT DISTINCT FROM"),
        }
    }
}
...
impl FromStr for JoinOperator {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        match s {
            "IS NOT DISTINCT FROM" | "is not distinct from" => Ok(Self::IsNotDistinctFrom),
            _ => bail!("invalid join operator `{s}`"),
        }
    }
}
